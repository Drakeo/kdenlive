    ...
    if (m_innerScopeRect.width() > 0 && m_innerScopeRect.height() > 0) {
        ...
        if (m_aGrid->isChecked()) {
            for (uint hz = 0; x <= rightBorder; hz += hzDiff) {
                ...
                hideText = m_aTrackMouse->isChecked() && m_mouseWithinWidget && fabs(x-(leftDist + mouseX + 20)) < (int) minDistX + 16
                        && mouseX < m_innerScopeRect.width() && mouseX >= 0;
            ...
            hideText = m_aTrackMouse->isChecked() && m_mouseWithinWidget && fabs(x-(leftDist + mouseX + 30)) < (int) minDistX
                    && mouseX < m_innerScopeRect.width() && mouseX >= 0;
            davinci.drawLine(x, topDist, x, topDist + m_innerScopeRect.height()+6);
            if (!hideText) {
